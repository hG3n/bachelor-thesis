...
Zu Beginn des Algorithmus muss der durch die Basislinie verursachte nicht matchbare Bereich entfernt werden. Dieser Schritt dient einerseits zur Veränderung der Disparitätenkarte  Dies geschieht nach der Wahl der in (\ref{subsec:stereo_matching_sgbm}) beschriebenen Parameter der SGBM. Dieser sogenannte Pixelshift berechnet sich aus der Hälfte der Anzahl der zu berechnenden Disparitäten. Sollte dieser einen ungeraden Wert ergeben so wird er um 1 erhöht. Generell ist der Pixelshift so gewählt das die Dimensionen der Disparity Map durch 8 Teilbar sind. Dieser Wert hat sich während der Entwicklung als robuster Wert erwiesen.\\
